...
# We have seen that we can achieve reasonable performance on an image classification task by training a linear classifier
#     on the pixels of the input image.
# In this exercise we will show that we can improve our classification performance by training linear classifiers
...
# train SVMs on top of the features extracted above; this should achieve better results than training
# SVMs directly on top of raw pixels.
...
# Earlier in this assigment we saw that training a three-layer neural network on raw pixels
# achieved better classification performance than linear classifiers on raw pixels.
...
# This approach should outperform all previous approaches:
# you should easily be able to achieve over 55% classification accuracy on the test set;
...
for batch_size in [256]:
    for lr in [1e-3]:
        for lrdecay in [0.99]:
            for reg in [0.001]:
                net = ThreeLayerNet(input_dim, hidden_dim, num_classes)
                print(batch_size, lr, lrdecay, reg)
                with open('train_result.txt', 'a+') as f:
                    ...
                    try:
                        stats = net.train(X_train_feats, y_train, X_val_feats, y_val,
                                          ...
                                          learning_rate=lr, learning_rate_decay=lrdecay,
                                          reg=reg, verbose=False)
                    except Exception as e:
                        print(e)
                        # print(stats)
                        continue
                    # net.train(X_train, y_train, X_val, y_val, num_iters=15000, batch_size=batch_size, learning_rate=lr,
